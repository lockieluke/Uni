...
if (!JWT && !IS_DEVELOPMENT) throw new Error("JWT is not set. Please set the JWT environment variable.");
...
if (!(await file.exists())) throw new Error(`File ${audioFilePath} does not exist`);
...
if (!response.ok) throw new Error(`Error: ${_.get(json, "error.message", "Unknown error occured")}`);
